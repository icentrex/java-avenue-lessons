...
//        snowBobCat.stopOnParking();
        ...
        Car[] cars = {lamborghini, miniBMW, snowBobCat};
        for (Car car : cars) {
            ...
            if (car instanceof SportCar) {
                ((SportCar) car).startWithTwoPedals();
                ((SportCar) car).raiseSpoiler();
            }

            if (car instanceof ChildCar) {
                ((ChildCar) car).playKidsMusic();
            }

            if (car instanceof SnowRemovalCar) {
                ((SnowRemovalCar) car).startSnowRemoval();
            }
        }
    }
}
